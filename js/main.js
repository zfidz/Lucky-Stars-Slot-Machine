...
function onSpinClicked() {
	...
	if (currentBet === 0) {
	  ...
	  return setTimeout(() => document.getElementById('notification').innerHTML = '', 1500);
	}
	...
	for (let i = 0; i < 3; i++) {
	  ...
	  const interval = setInterval(() => {
		...
		if (j >= 30) {
		  ...
		  symbols[i].style.backgroundImage = `url('images/${images[numbers[i]-1]}.png')`;
		}
	  }, 25 * (i+1));
	}

	setTimeout(() => {
	  const winCombos = [[1,1,1],[2,2,2],[3,3,3]];
	  const winAmts = [4, 6, 10];

	  const index = winCombos.findIndex(combo => combo.every((num, i) => num === numbers[i]));
	  if (index !== -1) {
